import React, { useState, useEffect, useMemo, useRef } from "react";
   ...
   import { Plus, Copy, Save, Repeat, Calendar, FileText, Search, CheckCircle, Stamp, MessageSquare, Trash2, X, Edit, ArrowRight, ArrowLeft, ChevronDown, ChevronUp, Filter, Check, ClipboardPaste } from "lucide-react";
   ...
   export default function DeviceAssignments() {
     ...
     const handleBulkPaste = () => {
        ...
        inputNumbers.forEach(num => {
          const device = allHeadsets.find(d => Number(d.binocular_number) === num);
          if (device) {
        ...
        if (notFoundNumbers.length > 0) {
          toast({
            ...
            description: `שים לב: המשקפות הבאות לא נמצאו: ${notFoundNumbers.join(", ")}`,
            variant: "warning",
            duration: 6000,
          });
        } else {
                     ...
                     <div className="flex items-center justify-between flex-wrap gap-4">
                       ...
                       {!isReadOnly && (
                         <div className="flex gap-2">
                           ...
                           <Button onClick={() => setShowPasteModal(true)} variant="outline" className="border-purple-200 hover:bg-purple-50 text-purple-700 gap-2 h-9">
                             <ClipboardPaste className="w-4 h-4" /> יבא מרשימה
                           </Button>
                         ...
                         <div className="flex items-center justify-between flex-wrap gap-4">
                           ...
                           {!isReadOnly && (
                             <div className="flex gap-2">
                               ...
                               <Button onClick={() => { setCurrentSessionIndex(idx); setShowPasteModal(true); }} size="sm" variant="outline" className="border-cyan-200 hover:bg-cyan-50 text-cyan-700 gap-2">
                                 <ClipboardPaste className="w-4 h-4" /> יבא רשימה
                               </Button>
